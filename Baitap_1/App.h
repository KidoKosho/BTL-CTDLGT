...
void App::Menu(){//done
    ...
    do{
        cin >> chon;
        if(chon<1 ||chon>7) cout <<"Nhap lai gia tri tu 1->7:";
    }while(chon<1 ||chon>7);
    if(chon == 7) return;
    else if(chon == 1) Them();
    else if(chon == 2) Xoa();
    ...
    else if(chon ==6) Thaydoithongtin();
}
void App::Them(){//done
    this_thread::sleep_for(chrono::milliseconds(500));
    cout <<"-----------------\n";
...
void App::Thaydoithongtin(){
    listsv.ThaydoithongtinSinhVien();
    Menu();
}
#endif